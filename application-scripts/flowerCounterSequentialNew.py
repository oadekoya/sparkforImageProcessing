...
class FlowerCounter(object):
    ...
    def computeAverageHistograms(self):
        ...
        try:
            ...
            avg_hist_b = np.divide(avg_hist_B, len(self.hist_b_all))
    ...
    def computeHistogramShifts(images):
        ...
        for image in tqdm(images, file=sys.stdout):
            try:
                ...
                correlation_b = np.correlate(self.hist_b_all[image], self.avg_hist_b, "full")
    ...
    def computeFlowerPixelsPercentage(self, images):
        ...
        for image in tqdm(images, file=sys.stdout):
            try:
                ...
                n_flower_pixels = np.sum(hist_b[self.segm_B_lower + self.hist_b_shift: self.segm_B_upper + self.hist_b_shift + 1])
...
if __name__ == "__main__":
    ...
    pool.map(flowerCounter.runner, chunks(image_filenames,4))
